...
class Category(object):
    ...
    def create_category(name, user_id, parent_id):
        ...
        category = CategoryModel(name=name,created_by=user_id, parent_id=parent_id)
        try:
            category.save()
            response = jsonify({
                'id': category.id,
            ...
            response = jsonify({'Error': 'Category' + category.name.capitalize() + 'already exists, add a sub category to it'})
    ...
    def get_categories( user_id,search, limit):
        ...
        response = CategoryModel.query.filter_by(created_by=user_id).limit(limit).all()
        if not response:
            response = jsonify([])
            ...
            if search:
                categories = CategoryModel.query.filter_by(
                    created_by=user_id).filter(CategoryModel.name.ilike('%{0}%'.format(search)))
            else:
                categories = CategoryModel.query.filter_by(created_by=user_id)
            
            if categories:

                pagination = categories.paginate(page, per_page=limit, error_out=False)
                category_lists = pagination.items
                if pagination.has_prev:
                    prev = url_for('get_categories', page=page-1, limit= limit, _external=True)
                else:
                    prev = None
                if pagination.has_next:
                    next = url_for('get_categories', page=page+1, limit=limit, _external=True)
                else:
                    next = None
                if category_lists:
