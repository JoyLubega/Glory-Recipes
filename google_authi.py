...
def get_user_info():
    ...
    return {"token": flask.session[AUTH_TOKEN_KEY]['id_token'],
            "user": oauth2_client.userinfo().get().execute()
        }
...
def index1():
    if is_logged_in():
        ...
        print(user_info, "hey")
        return '<div>You are currently logged in as ' + user_info['user']['given_name'] + '<div><pre>' + json.dumps(user_info, indent=4) + "</pre>"
